...
import './ResponsiveMode.css';

export default function ResponsiveMode() {
  const { data } = getData();
  ...
  return (
    <div className="wx-TCEks3sn demo">
      <h4>
        DataGrid can have different settings for different container width
      </h4>
      <Button
        type="primary"
        className="wx-TCEks3sn btn-change-width"
      ...
      <HeaderMenu api={api}>
        <div style={{ width: widths[index], marginTop: '10px' }}>
          <Grid
            init={setApi}
            data={data}
            columns={columns}
